...
@app.route('/preh')
def preh():
	return render_template('preh.html')
...
def checkd():
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute('SELECT * FROM diabetes WHERE name = %s', (na))
        det = cursor.fetchall()
        print(det)
        ...
        return render_template('preds.html',list=li,len=lt)
    return render_template('home.html')
...
def checkh():
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute('SELECT * FROM hdp WHERE name = %s ', (na))
        det = cursor.fetchall()
        ...
        return render_template('prehs.html',list=li,len=lt)
    return render_template('home.html')
